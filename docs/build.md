...
| `installer` | `live` | `base hardware domain desktop installer` | Live installer medium carrying the `desktop` payload |
...
- The `installer` profile requires a completed `desktop` build at the same `VERSION`: it stages the desktop profile's root EROFS, UKI and `/var` tarball as the payload it writes to target disks.
...
| `out/immos_<version>[-<profile>].root.erofs` | Root filesystem image |
| `out/uki[-<profile>]/immos_<version>.efi` | Unified kernel image |
| `out/immos_<version>[-<profile>].var.tar.zst` | `/var` template |

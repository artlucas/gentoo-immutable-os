...
| 30 | `scripts/stages/30-target-rootfs.sh` | The two-root emerge: build-time dependencies merge into the builder's own `/`, runtime dependencies merge into `$TARGET`; when the live target is not a merge base (absent, no package database, or stale membership) restore it from the snapshot the stage keeps of itself, then merge the delta and unmerge every atom the current lock no longer names; verify the resulting package database against the lock in both directions |
...
| Target-root staleness | `target_closure_hash()` against `/work/target-config-hash*` | The target root carries package membership the current config would drop — restored from the snapshot when one is valid, refused only when none is; then remove the work volume and rebuild — see [Build cache](cache.md#invalidation) |
...
The target-root guard dies only when no restore happened, because the restore path answers the objection the guard exists for. Stage 30 keeps a snapshot of the target in the work volume beside it (`/work/target-snap[-<profile>]`, one generation, rewritten after every successful run): a copy exactly as the stage left it, package database intact, nothing configured, nothing pruned. When the live target is absent, has no package database (stage 50 deletes it at the end of every build), or is stale by the closure hash, stage 30 restores the snapshot, merges the delta with `--changed-use`, and then reconciles — unmerging every atom the current lock no longer names, existence-guarded, before the bidirectional lock verify runs. `--changed-use` never removes packages; the unmerge does, for packages dropped from the graph, while a file dropped from a kept package is a USE change and rebuilds. An invalid or absent snapshot logs one warning and degrades to the pre-snapshot behaviour; `NO_TARGET_SNAPSHOT=1` disables both halves. The whole mechanism is `rsync`, `rm` and one atomic `mv` inside the work volume, so it needs nothing of the host and behaves identically under rootless Podman.

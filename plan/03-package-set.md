...
The native set is the **smallest thing that is still a complete KDE Plasma Wayland desktop
...
Notes: `-handbook` drops KHelpCenter docs; `-webengine` keeps QtWebEngine (a ~2h build and
~300 MB) out of the native image — anything needing a browser engine lives in Flatpak.
`-semantic-desktop` drops Baloo file indexing (heavy; optional roadmap re-add). X11 *libraries*
remain (toolkits need them) but the **Xorg server is excluded — Wayland-only, with
`x11-base/xwayland` for legacy-app compat inside the session.**

## Native package set (Portage sets in `config/portage/sets/`)

### @arttest-base — boot + plumbing
...
| sys-apps/dbus, sys-auth/polkit | desktop/system IPC + privilege broker (Flatpak, NM, Discover all need polkit) |
...
| sys-apps/flatpak, sys-apps/xdg-desktop-portal, kde-plasma/xdg-desktop-portal-kde | the application layer |
...
### @arttest-hardware — kernel, firmware, drivers
...
### @arttest-desktop — minimal Plasma

| Package | Why |
|---|---|
| kde-plasma/plasma-desktop | the shell |
| kde-plasma/plasma-nm, plasma-pa, powerdevil, kscreen, bluedevil | network/audio/power/display/BT applets — the "it works like a laptop" set |
| kde-plasma/breeze, breeze-gtk, kde-frameworks/breeze-icons | theme so GTK flatpaks don't look alien |
| kde-plasma/systemsettings, kde-plasma/plasma-systemmonitor | control panel + task manager |
| kde-plasma/kwin (pulled by plasma-desktop; `USE="lock"`) + kde-plasma/kscreenlocker | compositor, lock screen |
| x11-misc/sddm (Wayland greeter mode: `DisplayServer=wayland`) + kde-plasma/sddm-kcm | login/autologin |
| kde-apps/konsole, kde-apps/dolphin, kde-apps/ark, kde-apps/kwrite | the four native utilities: terminal, files, archives, text. Native (not Flatpak) because they need unrestricted host access |
| kde-plasma/discover (`USE="flatpak -packagekit -firmware"`) | GUI software center, **Flatpak backend only** — it cannot even offer native packages |
...
| net-print/cups + kde-plasma/print-manager (`build.conf` switch, default **on**) | printing can't be flatpak'd |
...
update path, listed in `@arttest-base`.
...
- Portals: xdg-desktop-portal-kde gives flatpaks file pickers, screenshot/screencast,
  settings. This is the part that makes "minimal native, everything Flatpak" actually pleasant.

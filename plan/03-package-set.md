...
| *(boot splash — **no package**)* | `sys-boot/plymouth` was removed in [plan/14](14-boot-splash-kms.md). The splash is now `config/splash/splash.c`, ~800 lines compiled `-static` by stage 40 and installed as `/usr/bin/<id>-splash`, plus one file of pre-composited pixels in `/usr/share/<id>/`. It needs no text engine, no image decoder and no libdrm — the DRM ioctls are the kernel uapi, used directly. Three build-only companions, `gnome-base/librsvg[tools]` (`rsvg-convert`), `media-fonts/ibm-plex` and `dev-python/pillow`, live in `builder/Dockerfile` and never enter the image. This is the repo's only compiled artifact; the toolchain-free guarantee in [plan/06](06-pruning.md) is unaffected, since the compiler is the *builder's* and `-static` means the result depends on nothing the image ships |
...
| media-video/pipewire (`USE="sound-server pipewire-alsa"`) + media-video/wireplumber | audio/video routing. `sound-server` is what builds `pipewire-pulse`, the PulseAudio-protocol daemon `plasma-pa` actually talks to via `media-libs/pulseaudio-qt`. **Installing it does not start it, and the profile does not either** — this row said "(profile default)" until 0.3.0 shipped mute. PipeWire is a per-user service and enables nothing itself; the ebuild wraps its `/etc/xdg/autostart` file and `gentoo-pipewire-launcher` in `if ! use systemd`, so on this profile the systemd **user** units are the only start path, and `/etc/pulse/client.conf` ships `autospawn = no` so libpulse cannot recover. Stage 40 enables `pipewire.socket`, `pipewire-pulse.socket` and `wireplumber.service` with a targeted `systemctl --global enable` — *not* `--global preset-all`, which (no catch-all user preset in Gentoo → systemd's built-in "enable" policy) also pulls in `podman.socket` and friends against plan/13's rootless-only rule |

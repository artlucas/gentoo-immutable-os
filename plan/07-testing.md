...
| **T-MAN-4** | **The install that cannot reach the control plane.** The page and the job exist (Phase D); the guest-level run is blocked on the same unattended-Calamares work as T-DOM-1. Asserted offline meanwhile, on the two properties that carry it: the view step always allows *next*, and the job has no path that returns a failure tuple — a Calamares python job fails an install by returning one, so a single `return (` in that file is an installer that dies because a household's router was being replaced |

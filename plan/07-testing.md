...
| **T-MAN-4** | **The install that cannot reach the control plane.** Rewritten by [plan/21](../plan/21-installer-accounts-page.md) §8 into two halves — the page refusing to continue against a dead API, and an install completing from a bundle fetched before the disk was written. The guest-level run is still blocked on the same unattended-Calamares work as T-DOM-1. Asserted offline meanwhile, on the properties that carry it: `accountsetup`'s network-facing paths have no `return (` at all (a Calamares python job fails an install by returning a tuple, so one there is an installer that dies because a household's router was being replaced), while its local-account path deliberately does have one — a `useradd` that fails leaves nothing to recover with, and that is the one failure worth stopping for |

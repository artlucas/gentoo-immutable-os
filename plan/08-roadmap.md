...
- **Installer:** Calamares (KDE-native, proven, modular) with a thin custom module that:
  ...
  removes the baked live user. Alternative considered: a purpose-built KDE/Kirigami installer
  — more work, revisit after Calamares spike.
...
- **Discover integration:** OS update visibility in the GUI next to Flatpak updates
  (Discover has a generic backend API; SteamOS does similar).
...
- **fwupd** — UEFI capsule + device firmware updates; needs an ESP-write policy and
  `esp` mount coordination with sysupdate; straightforward addition to @arttest-hardware.
...
| No 3-way `/etc` merge (overlay upper shadows vendor changes) | OSTree-grade merge machinery is not worth v1 complexity; `arttest-update etc-diff` gives visibility |
...
| Native Qt/KDE apps limited to konsole/dolphin/ark/kwrite | Everything else Flatpak — the point of the distro; portals make it seamless |
| Generic x86-64 (no AVX2 floor) | Budget Atom-class CPUs sold within the window lack AVX2 |
| QtWebEngine excluded natively | Biggest single build/system-size win; browser ships as Flatpak Firefox |
...
4. SDDM Wayland greeter stability on NVIDIA — if flaky, greeter falls back to X11 (keeps
   xorg-server out only if `sddm`'s Wayland mode holds; else revisit the no-Xorg stance for
   the greeter alone).

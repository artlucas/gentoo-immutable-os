/*
 * VENDORED FROM CALAMARES 3.4.2, src/modules/welcome/checker/ResultsListWidget.cpp (plan/23 §2).
 *
...
ResultsListWidget::ResultsListWidget( GreetingConfig* config, QWidget* parent )
    : QWidget( parent )
    , m_config( config )
{
    ...
    connect( config, &GreetingConfig::warningMessageChanged, m_explanation, &QLabel::setText );
}

 ...
 *
 * WHAT IS NOT ON THIS PAGE and why — the Formats, Measurement, automatic-time and 24-hour
 * controls the design hand-off draws — is written out in LocationConfig.h. Short version: two of
 * them would offer locales the image did not compile, and two are settings nothing in this
 * pipeline writes to the installed system.
 */
...
Item {
    ...
    ColumnLayout {
        ...
        Rectangle {
            Layout.fillWidth: true
            Layout.maximumWidth: ds.contentMaxWidth
            implicitHeight: clockRow.implicitHeight + 2 * (ds.space5 + 2)
            radius: ds.radiusLg

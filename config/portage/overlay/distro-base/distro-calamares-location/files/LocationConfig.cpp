...
LocationConfig::prettyStatus() const
{
    ...
    // kept system keeps its own time zone, and `localesetup` stands down entirely under keep —
    // so the summary row must say that rather than name a zone that will not be applied.
